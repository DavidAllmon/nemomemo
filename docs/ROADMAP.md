...
| ⭐ NEW **Telegram capture bot** | ✅ **v1.31.0** — `NEMOMEMO_TELEGRAM_BOT_TOKEN` + a long-polling service (migration 0011); a chat binds to a member by a single-use 15-minute code; text, photos, and voice notes become memos with `#tags` inline, media sharing the upload route's OCR/transcription via the extracted `storeAttachment()`. Started only on the single-tenant branch, so cloud can't run N pollers on one token. Discord **tabled** (David, 2026-08-25) — different job: shared/team reefs, not pocket capture. | M |
