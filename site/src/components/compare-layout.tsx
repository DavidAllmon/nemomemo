...
export function CompareLayout({ comparison }: { comparison: Comparison }) {
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-14">
      <p className="text-sm font-bold text-ocean-muted">
        <Link href="/compare" className="hover:text-ocean-ink">
          Compare
        </Link>{' '}
        / {comparison.name}
      </p>
      <h1 className="mt-2 font-display text-4xl font-bold">{comparison.heading}</h1>
      <p className="mt-3 text-lg text-ocean-muted">{comparison.subheading}</p>
