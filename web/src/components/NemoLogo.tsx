...
/** Inline-SVG clownfish mark. Scales with the `size` class you pass. */
export function NemoLogo({ className, bob = false }: { className?: string; bob?: boolean }) {
  return (
    <svg
      viewBox="0 0 64 48"
      className={cn('size-8', bob && 'animate-fish-bob', className)}
      role="img"
      aria-label="NemoMemo"
    >
      {/* tail */}
...
export function Wordmark({ className }: { className?: string }) {
  return (
    <span className={cn('font-display text-lg font-bold tracking-tight', className)}>
      Nemo<span className="text-primary">Memo</span>
    </span>
  );
}
